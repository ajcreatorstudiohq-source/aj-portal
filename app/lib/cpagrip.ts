/**
 * CPAGrip Offer Wall — direct show.php link (no script_include / content locker).
...
/** Production CPAGrip offerwall URL template (tracking_id = Firebase uid) */
...
export function buildCpaGripWallUrl(uid?: string | null): string {
  const base =
    process.env.NEXT_PUBLIC_OFFERWALL_URL || CPAGRIP_SHOW_BASE;
  try {
