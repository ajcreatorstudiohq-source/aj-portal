...
const [aiChat, setAiChat] = useState([{role:'bot', text:'Hello! Main AJ ka AI Assistant hoon. CEO ke behalf par aapki madad karunga.'}]);
...
// --- FETCH REAL CONTENT (YouTube + Unsplash + News) ---
const fetchSocialAPIs = async () => {
    try {
        const ytRes = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=10&q=shorts+trending+viral&type=video&videoDuration=short&key=${YT_API_KEY}`);
        const ytData = await ytRes.json(); setReels(ytData.items || []);

        const unRes = await fetch(`https://api.unsplash.com/photos/random?count=10&client_id=${UNSPLASH_KEY}&query=luxury,lifestyle,tech`);
        ...
        const nData = await nRes.json(); setNewsData(nData.articles?.slice(0, 10) || []);
    } catch (e) { console.log("API Error"); }
};

// --- ZEGO CALLING (WhatsApp Style) ---
...
// --- AI BOT LOGIC (CEO Representative) ---
const handleAiAssistant = () => {
    if(!aiInput.trim()) return;
    const msg = aiInput.toLowerCase();
    let res = "Main is masle ko hal nahi kar sakta. Please niche 'Talk with CEO' par click karein.";
    
    if(msg.includes('coin') || msg.includes('balance')) res = "Aap Wallet mein ja kar coins khareed sakte hain. 1000 coins $2 ke hain.";
    if(msg.includes('refer') || msg.includes('dost')) res = "Hub par apna ID copy karein, dost ke join karne par aapko 50 coins milenge.";
    if(msg.includes('withdraw')) res = "Minimum withdrawal 5000 coins hai. Binance Pay best hai.";
    if(msg.includes('gift')) res = "Gifting se aap creators ko support kar sakte hain. Coffee se Mansion tak options hain.";
...
useEffect(() => {
    if (user) {
        ...
        // Withdrawal History
        const wQ = query(collection(db, "withdrawals"), where("uid", "==", user.uid), orderBy("date", "desc"));
        ...
        onSnapshot(doc(db, "users", user.uid), (snap) => {
            if (snap.exists()) { 
                ...
                setTempPhoto(snap.data().photo || user.photoURL);
            }
        });
    }
}, [user]);
...
const handleApplyReferral = async () => {
    if(!referralInput.trim() || referralInput === user.uid) return alert("Invalid Referral ID");
    const refRef = doc(db, "users", referralInput);
    const snap = await getDoc(refRef);
    if(snap.exists()){
        await updateDoc(refRef, { balance: increment(50) });
        alert("✅ Referral Success! Your friend received 50 coins.");
        setReferralInput('');
...
useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
        if (currentUser) {
            ...
            if (!userSnap.exists()) {
                await setDoc(userRef, { username: currentUser.displayName.replace(/\s+/g,'').toLowerCase(), balance: 500, uid: currentUser.uid, photo: currentUser.photoURL, hasSocialProfile: true, date: serverTimestamp() });
                alert("🎁 +500 Coins Welcome Bonus!");
            }
            setScreen('hub');
        } else { setScreen('auth'); }
    });
...
const handleLike = async (id) => {
    ...
    await setDoc(doc(db, "content_meta", id), { likes: increment(1) }, { merge: true });
};

const handleShare = (msg) => { if(navigator.share) navigator.share({title:'AJ Portal', text: msg, url: window.location.href}); else alert("Link Copied!"); };

if (screen === 'splash') return <div className="h-screen bg-black flex flex-col items-center justify-center"><img src="/logo.png" className="w-32 animate-bounce" /><h1 className="text-cyan-400 font-black mt-4 tracking-widest">AJ PORTAL</h1></div>;
if (screen === 'auth' && !user) return <div className="h-screen bg-black flex flex-col items-center justify-center p-10"><h1 className="text-6xl font-black italic text-cyan-400 mb-10">AJ ID</h1><button onClick={() => signInWithPopup(auth, googleProvider)} className="bg-white text-black px-12 py-5 rounded-3xl font-black text-xl shadow-2xl">CONTINUE WITH GOOGLE</button></div>;
...
<main className="min-h-screen bg-[#020617] text-white font-sans overflow-x-hidden relative">
        ...
        <section className="min-h-screen flex flex-col items-center p-6 pt-32 pb-40">
            ...
            <div className="w-full max-w-2xl bg-gradient-to-r from-cyan-900/20 to-blue-900/20 border border-cyan-500/30 p-6 rounded-[2.5rem] mb-12 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl backdrop-blur-md">
                <div className="flex items-center gap-4 text-left">
                    ...
                    <div><h3 className="font-black text-lg uppercase">Refer & Earn 50 🪙</h3><p className="text-[10px] text-gray-400 font-bold uppercase">Give AJ ID to Friends</p></div>
                </div>
            ...
            <div className="grid grid-cols-2 gap-6 md:gap-16 w-full max-w-5xl z-30">
                <div onClick={() => setScreen('arcade')} className="bg-white/5 border border-white/10 rounded-[3rem] h-56 md:h-96 flex flex-col items-center justify-center cursor-pointer shadow-2xl hover:border-cyan-400 active:scale-95"><Trophy className="text-cyan-400 w-12 h-12 md:w-24 md:h-24 mb-4" /><span className="font-black text-sm md:text-4xl uppercase italic">Gaming</span></div>
                <div onClick={() => { fetchSocialAPIs(); setScreen('social'); setSocialScreen('hub'); }} className="bg-white/5 border border-white/10 rounded-[3rem] h-56 md:h-96 flex flex-col items-center justify-center cursor-pointer shadow-2xl hover:border-pink-500 active:scale-95"><Zap className="text-pink-500 w-12 h-12 md:w-24 md:h-24 mb-4" /><span className="font-black text-sm md:text-4xl uppercase italic">Social</span></div>
                <div onClick={() => setScreen('wallet')} className="bg-white/5 border border-white/10 rounded-[3rem] h-56 md:h-96 flex flex-col items-center justify-center cursor-pointer shadow-2xl hover:border-yellow-500 active:scale-95"><Wallet className="text-yellow-500 w-12 h-12 md:w-24 md:h-24 mb-4" /><span className="font-black text-sm md:text-4xl uppercase italic">Wallet</span></div>
                <div onClick={() => {setScreen('social'); setSocialScreen('ai_bot');}} className="bg-white/5 border border-white/10 rounded-[3rem] h-56 md:h-96 flex flex-col items-center justify-center cursor-pointer shadow-2xl hover:border-green-500 active:scale-95"><Bot className="text-green-400 w-12 h-12 md:w-24 md:h-24 mb-4" /><span className="font-black text-sm md:text-4xl uppercase italic">AJ AI</span></div>
            </div>
        </section>
    )}
    ...
    {screen === 'social' && (
        <div className="fixed inset-0 z-[400] bg-slate-950 flex flex-col h-screen overflow-hidden">
            <header className="p-4 bg-black/95 border-b border-white/10 flex justify-between items-center shadow-xl">
                <button onClick={() => setScreen('hub')} className="text-pink-500 font-black text-xs uppercase">← EXIT</button>
            ...
            <div className="flex-1 overflow-y-auto">
                {socialScreen === 'hub' ? (
                    <div className="max-w-md mx-auto grid grid-cols-1 gap-6 p-8 pb-32">
                        {[{n:'AJ TikReels', i:Video, s:'tikreels', d:'Viral YouTube Shorts'}, {n:'AJ Pulse', i:Users, s:'pulse', d:'Premium Lifestyle Feed'}, {n:'AJ WeChat', i:MessageSquare, s:'chatlist', d:'WhatsApp Style Messaging'}, {n:'AJ Discover', i:Globe, s:'discover', d:'Latest AI & Future Tech'}].map((mod) => (
                            <div key={mod.n} onClick={() => setSocialScreen(mod.s)} className="p-10 bg-white/5 border border-white/10 rounded-[3rem] text-center hover:border-pink-500 transition-all cursor-pointer shadow-2xl">
                                <mod.i size={44} className="text-pink-500 mx-auto mb-4" /><h3 className="text-2xl font-black uppercase italic">{mod.n}</h3><p className="text-[10px] text-gray-400 uppercase mt-2 font-bold">{mod.d}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="h-full w-full max-w-md mx-auto snap-y snap-mandatory overflow-y-auto bg-black">
                        {reels.map((vid, i) => (
                            <div key={i} className="h-[88vh] w-full snap-start relative border-b border-white/5">
                                ...
                                <div className="absolute right-4 bottom-32 flex flex-col gap-8 items-center z-50">
                                    ...
                                    <MessageCircle size={38} className="text-white" onClick={() => setCommentBoardPostId(vid.id.videoId)}/>
                                    <Zap size={38} className="text-yellow-500" onClick={() => setShowGiftModal(vid.snippet.channelId)}/>
                                    <Share2 size={38} className="text-white" onClick={() => handleShare('Watch this viral short on AJ Portal!')}/>
                                </div>
                            </div>
                        ))}
                    </div>
                        ...
                        {['AJ Support Center', 'CEO VIP Room', 'Global AJ Community'].map((name, i) => (
                            <div key={i} onClick={() => setSocialScreen('chat')} className="flex items-center gap-4 p-5 hover:bg-white/5 cursor-pointer border-b border-white/5 rounded-[2rem] mx-2 transition-all">
                                ...
                                <div className="flex-1 text-left">
                                    ...
                                    <p className="text-[10px] text-gray-500 line-clamp-1 italic">Encrypted WeChat session active...</p>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="max-w-md mx-auto h-[88vh] flex flex-col bg-[#0b141a] m-2 rounded-[3.5rem] overflow-hidden shadow-2xl border border-cyan-500/20">
                        ...
                        <div className="flex-1 p-6 bg-[url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')] bg-contain">
                             <div className="p-4 bg-[#202c33] text-white rounded-2xl rounded-tl-none border border-white/5 shadow-xl text-xs">Assalamu Alaikum! AJ Portal Support mein swagat hai. Main AJ AI Assistant hoon. Main aapki kya madad kar sakta hoon?</div>
                        </div>
                        <div className="p-4 bg-[#1f2c33] flex gap-3 shadow-inner"><PlusSquare size={28} className="text-[#aebac1] cursor-pointer" /><input type="text" placeholder="Type a message" className="flex-1 bg-[#2a3942] border-none p-4 rounded-full text-xs text-white outline-none font-bold" /><button className="bg-cyan-600 p-4 rounded-full text-white shadow-xl active:scale-90 transition-all"><Send size={22}/></button></div>
                    </div>
                    ...
                    <div className="h-full w-full bg-black relative"><div id="call-container" className="w-full h-full"></div><button onClick={()=>setSocialScreen('hub')} className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-red-600 px-12 py-5 rounded-full font-black uppercase text-sm z-[2000] border-4 border-white/10">Hang Up</button></div>
                ) : null}
            </div>
            ...
            <div className="w-full max-w-md bg-[#111] border-2 border-white/10 p-12 rounded-[3.5rem] text-center shadow-2xl relative overflow-hidden">
                ...
                {walletTab === 'main' ? (
                    ...
                    <div className="flex flex-col gap-8 text-left">
                        <div className="space-y-4"><label className="text-[10px] font-black text-cyan-400 uppercase tracking-[0.4em] ml-2">Recipient ID</label><input type="text" value={transferId} onChange={(e)=>setTransferId(e.target.value)} placeholder="Paste Receiver AJ ID" className="w-full bg-black/60 border-2 border-white/5 p-6 rounded-[2rem] text-white outline-none focus:border-cyan-500 font-bold shadow-inner" /></div>
                        <div className="space-y-4"><label className="text-[10px] font-black text-cyan-400 uppercase tracking-[0.4em] ml-2">Amount (Coins)</label><input type="number" value={transferAmount} onChange={(e)=>setTransferAmount(Number(e.target.value))} className="w-full bg-black/60 border-2 border-white/5 p-6 rounded-[2rem] text-white outline-none font-black text-3xl text-center shadow-inner" /></div>
                        <button onClick={async () => { if(balance < transferAmount || transferAmount < 1) return alert("Invalid Amount!"); await updateDoc(doc(db,"users",user.uid),{balance:increment(-transferAmount)}); await updateDoc(doc(db,"users",transferId),{balance:increment(transferAmount)}); alert("Transfer Success! 🪙"); setWalletTab('main'); }} className="bg-cyan-500 py-7 rounded-[2.5rem] font-black uppercase text-black shadow-2xl active:scale-95 transition-all text-xl">SEND COINS</button>
            ...
            <div className="w-full max-w-sm bg-[#111] border-2 border-cyan-500/30 rounded-[3rem] p-8 shadow-2xl">
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                    {notifications.length > 0 ? notifications.map((n, i) => (
                        <div key={i} className="bg-white/5 p-4 rounded-2xl border border-white/5 shadow-md"><p className="text-xs font-black text-white uppercase mb-1">{n.title || "AJ Portal Update"}</p><p className="text-[10px] text-gray-400 italic">"{n.message || "Something new is live!"}"</p></div>
           ...
           <div className="w-full max-w-2xl bg-white/5 border-2 border-green-500/40 p-10 rounded-[4rem] text-center shadow-2xl relative">
                ...
                {botTier !== 'none' ? (
                    <div className="bg-black/50 p-8 rounded-3xl border border-green-500/20 text-left font-mono">
                        ...
                        <div className="text-[10px] text-green-500/50 leading-loose uppercase tracking-widest italic animate-pulse">Analysing market... Link established... Profit sync complete.</div>
                    </div>
                ) : (
    ...
    <footer className="bg-black py-40 px-10 border-t-2 border-white/5 text-center flex flex-col items-center relative overflow-hidden mt-10">
        ...
        <div className="text-7xl md:text-[11rem] font-black italic text-cyan-400 drop-shadow-[0_0_60px_rgba(6,182,212,0.5)] mb-16 uppercase tracking-tighter relative z-10 select-none">AJ STUDIO</div>
        ...
        <div className="flex flex-col items-center gap-10 mb-24 relative z-10">
            <a href="mailto:ajcreatorstudio.hq@gmail.com" className="flex items-center gap-6 bg-white/5 px-14 py-6 rounded-[2.5rem] border-2 border-cyan-500/20 hover:border-cyan-500 transition-all group shadow-2xl backdrop-blur-md">
                ...
                <span className="font-black text-sm md:text-xl tracking-widest text-white uppercase italic">ajcreatorstudio.hq@gmail.com</span>
            </a>
            <div className="flex justify-center gap-12 mt-4">
                <a href="https://wa.me/96878994093" target="_blank" className="flex items-center gap-3 text-green-500 border-2 border-green-500/20 px-10 py-4 rounded-full font-black uppercase hover:bg-green-500 hover:text-black transition-all shadow-xl text-sm tracking-widest text-white">WhatsApp</a>
                <a href="https://x.com/Ali20352061" target="_blank" className="flex items-center gap-3 text-white border-2 border-white/20 px-10 py-4 rounded-full font-black uppercase hover:bg-white hover:text-black transition-all shadow-xl text-sm tracking-widest text-white">Twitter (X)</a>
            </div>
        </div>
