...
import { MessageCircle, Trophy, Zap, Wallet, Bot, LogOut, Globe, ChevronRight, X } from 'lucide-react';
...
export default function AJSuperPortal() {
  ...
  // --- Logic 1: 100 Coins = $1 USDT ---
  const usdtBalance = (balance / 100).toFixed(2);

  // --- Logic 2: Splash Timer ---
  useEffect(() => {
    if (screen === 'splash') {
  ...
  // --- Logic 3: Real-time Firebase Sync ---
  useEffect(() => {
  ...
  // --- Logic 5: NowPayments Purchase ---
  const handlePurchase = () => {
    // API KEY: 3THXNSZ-AYVMTP6-HQ9KGKK-9J6CQD7
    const paymentUrl = `https://nowpayments.io/payment/?api_key=3THXNSZ-AYVMTP6-HQ9KGKK-9J6CQD7&amount=13&currency=usd&order_id=${user?.uid}_${Date.now()}`;
    ...
    return (
      <main className="min-h-screen bg-[#050505] flex flex-col items-center justify-center p-6 text-white text-center">
        <div className="w-full max-w-sm bg-white/[0.03] border border-white/10 p-12 rounded-[3rem] shadow-2xl">
          ...
          <button onClick={handleLogin} className="w-full py-5 bg-white text-black font-black text-xl rounded-2xl active:scale-95 transition-all shadow-xl">
            CONTINUE WITH GOOGLE
          </button>
          <div className="mt-8 text-yellow-500 font-bold text-xs animate-bounce uppercase">+500 COINS BONUS ON SIGNUP</div>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-white font-sans overflow-x-hidden">
      ...
      {/* VVIP HEADER: Real-time Balance (Coins + USDT) */}
      <header className="fixed top-0 w-full p-4 flex justify-between items-center z-[100] bg-black/60 backdrop-blur-xl border-b border-white/5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-4 bg-white/5 px-5 py-2 rounded-full border border-white/10 shadow-2xl">
            <div className="text-right leading-none border-r border-white/10 pr-3">
               <p className="text-[10px] font-black text-yellow-500 uppercase">{balance} COINS</p>
               <p className="text-sm font-black text-green-400 tracking-tighter">${usdtBalance} USDT</p>
            </div>
            <img src={user?.photoURL} className="w-9 h-9 rounded-full border-2 border-cyan-500" alt="u" />
          </div>
          <button onClick={handleLogout} className="p-2 bg-red-500/10 border border-red-500/30 rounded-full text-red-500 hover:bg-red-500 hover:text-white transition-all"><LogOut size={18} /></button>
        </div>
      </header>
      ...
      <section className="min-h-screen flex flex-col items-center justify-center p-4 relative pt-24">
        ...
        <div className="relative z-20 grid grid-cols-2 gap-4 md:gap-24 w-full max-w-4xl px-2">
          ...
          {/* Card: Wallet (Click to Buy) */}
          <div onClick={handlePurchase} className="bg-white/5 border border-yellow-500/30 rounded-[2rem] p-6 h-48 md:h-80 flex flex-col items-center justify-center active:scale-95 transition-all cursor-pointer shadow-[0_0_30px_rgba(234,179,8,0.1)] relative z-30">
            <img src="/gold.jpg" alt="C" className="w-14 h-14 md:w-44 md:h-44 object-contain mb-2 drop-shadow-[0_0_15px_#eab308]" />
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 opacity-40">
            <div className="w-20 h-20 md:w-80 md:h-80 bg-black border-2 md:border-8 border-cyan-500 rounded-full flex items-center justify-center shadow-[0_0_60px_#06b6d4]">
              <span className="text-[6px] md:text-6xl font-black text-white italic">HUB</span>
            </div>
          </div>
        </div>

        {/* Success Loop Bar */}
        <div className="mt-12 md:mt-24 bg-white/5 px-8 py-3 rounded-full flex gap-4 md:gap-16 text-[8px] md:text-2xl font-black text-gray-500 uppercase tracking-widest border border-white/5 shadow-2xl">
      ...
      <footer className="bg-black py-24 px-10 border-t border-cyan-500/10 text-center">
        <div className="max-w-7xl mx-auto flex flex-col items-center gap-10">
          <div className="text-7xl md:text-[10rem] font-black italic tracking-tighter text-white drop-shadow-[0_0_30px_rgba(34,211,238,0.5)]">
            AJ STUDIO
          </div>
          <div className="flex gap-14 mt-4">
               ...
               <a href="https://www.instagram.com/innocent.a.jutt" target="_blank" className="text-pink-500 text-6xl font-black italic hover:scale-125 transition-all">i</a>
          </div>
          <p className="mt-12 text-gray-700 font-black tracking-[1.5em] text-[10px] uppercase"><Globe size={12} className="inline mr-2"/> Muscat, Sultanate of Oman</p>
        </div>
      </footer>
        ...
        <div className="fixed inset-0 z-[200] bg-black flex flex-col items-center p-8 overflow-y-auto">
           ...
           <div className="flex flex-col gap-6 w-full max-w-md">
             {['AJ TikReels', 'AJ Pulse', 'AJ Live Chat'].map((module, i) => (
               <div key={module} className="flex items-center justify-between bg-white/[0.03] border border-white/10 p-10 rounded-3xl hover:border-pink-500 active:scale-95 cursor-pointer group shadow-2xl">
