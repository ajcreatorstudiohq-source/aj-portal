...
import { MessageCircle, Trophy, Zap, Bot, Send, Activity, CheckCircle2, Download, Copy, Video, Newspaper, Users, MessageSquare, Camera, Settings, Edit3, X, Mail, Lock, User, Globe } from 'lucide-react';
...
const handleManualSignup = async () => {
    ...
    try {
        ...
        alert("Account Created! Now setup profile details.");
...
const enterSocialMode = (mode: string) => {
    setPendingMode(mode);
    if (!hasSocialProfile) {
        setSocialScreen('setup'); // Force Login/Setup within mode
    } else {
        setSocialScreen(mode);
    }
};

// --- PROFIT LOGIC (70/30 + 100:1 Game) ---
useEffect(() => {
...
const safePortalCoins = rawReward / 100; // 100:1 Internal Conversion

const userRef = doc(db, "users", user.uid);
const adminRef = doc(db, "admin_ledger", "platform_stats");

if (data.type === 'EARNED' || data.type === "ADD_AD_REVENUE") {
    updateDoc(userRef, { balance: increment(safePortalCoins * 0.30) });
    updateDoc(adminRef, { total_revenue: increment(safePortalCoins * 0.70) });
  }
};
...
if (userSnap.exists()) {
  ...
  await setDoc(userRef, { name: currentUser.displayName || "New User", email: currentUser.email, balance: 500, botTier: 'none', invested: 0, uid: currentUser.uid, lastSync: serverTimestamp(), hasSocialProfile: false, photo: currentUser.photoURL });
}
...
{user && <img src={tempPhoto || user.photoURL} className="w-8 h-8 rounded-full border border-cyan-500 shadow-[0_0_10px_#06b6d4]" alt="DP" />}
</div>
    ...
    <div className="fixed inset-0 z-[400] bg-slate-950 p-8 overflow-y-auto">
        ...
        {socialScreen === 'hub' ? (
          <div className="max-w-md mx-auto grid grid-cols-1 gap-6 pb-24 px-2">
             <div className="flex items-center gap-3 bg-white/5 p-4 rounded-3xl border border-pink-500/20 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-gray-500 uppercase tracking-widest">{hasSocialProfile ? "Profile Verified" : "Setup Identity"}</p>
                  </div>
             </div>
          ...
          <div className="max-w-md mx-auto bg-white/5 border border-white/10 p-10 rounded-[3rem] text-center mt-4 shadow-2xl">
              ...
              <h2 className="text-xl font-black text-white mb-6 uppercase tracking-tighter italic">Social Identity</h2>
              ...
              {!user ? (
                  ...
                  <div className="space-y-4 text-left">
                      ...
                      <button onClick={handleCreateProfile} className="w-full mt-8 py-5 bg-pink-600 rounded-2xl font-black uppercase shadow-lg active:scale-95 transition-all">START SOCIALIZING</button>
                  </div>
              )}
...
{/* BAAKI MODALS: ARCADE, WALLET, FOOTER (AS IS) */}
{screen === 'arcade' && selectedGame && (
    <div className="fixed inset-0 z-[300] bg-black p-8 overflow-y-auto">
        <button onClick={() => setSelectedGame(null)} className="text-cyan-400 font-bold mb-10 tracking-widest uppercase transition-all hover:brightness-125">← BACK</button>
