...
export default function AJSuperPortal() {
  ...
  // Inputs
  const [purchaseOMR, setPurchaseOMR] = useState(5);
    ...
    <main className="min-h-screen bg-[#020617] text-white font-sans overflow-x-hidden relative">
      ...
      <section className="min-h-screen flex flex-col items-center justify-center p-4 pt-24 relative">
        ...
        <div className="relative z-20 grid grid-cols-2 gap-4 md:gap-16 w-full max-w-4xl px-2">
          ...
          <div onClick={() => { setScreen('wallet'); setWalletTab('main'); }} className="bg-white/5 border-2 border-yellow-500/30 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center active:scale-95 shadow-xl relative z-30 cursor-pointer">
            <img src="/gold.jpg" className="w-14 h-14 mb-2" />
            <h2 className="font-black text-xs md:text-3xl uppercase text-yellow-500">Wallet</h2>
          </div>
          <div onClick={() => setScreen('ai')} className="bg-white/5 border border-white/10 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center active:scale-95 shadow-xl relative z-30 cursor-pointer">
            <Bot className="mb-2 text-green-400 w-10 h-10 md:w-24 md:h-24" />
            <h2 className="font-black text-xs md:text-3xl uppercase font-black text-green-400">AJ AI</h2>
          </div>
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 opacity-40">
            <div className="w-20 h-20 md:w-80 md:h-80 bg-black border-2 md:border-8 border-cyan-500 rounded-full flex items-center justify-center shadow-[0_0_80px_#06b6d4]">
      ...
      <footer className="bg-black py-24 px-10 border-t border-cyan-500/10 text-center">
        <div className="text-7xl md:text-9xl font-black italic text-cyan-400 drop-shadow-[0_0_30px_#06b6d4] mb-12">AJ STUDIO</div>
        <div className="flex justify-center gap-14">
            ...
            <a href="https://x.com/Ali20352061" target="_blank" className="text-white text-5xl font-black italic">X</a>
        </div>
      </footer>
           ...
           <div className="w-full max-w-md bg-[#111] border border-white/10 p-10 rounded-[3rem] text-center shadow-2xl">
              ...
              {walletTab === 'main' && (
                <div className="flex flex-col gap-4">
                   <button onClick={()=>setWalletTab('purchase')} className="bg-white text-black py-5 rounded-2xl font-black text-lg">PURCHASE</button>
                   <button onClick={()=>setWalletTab('transfer')} className="bg-white/10 text-cyan-400 py-5 rounded-2xl font-black border border-cyan-500/30 text-cyan-400">TRANSFER</button>
                   <button onClick={()=>setWalletTab('withdraw')} className="bg-white/10 text-pink-500 py-5 rounded-2xl font-black border border-pink-500/30 text-pink-500">WITHDRAW</button>
                </div>
              )}
              {walletTab === 'purchase' && (
                <div className="flex flex-col gap-5">
                   <p className="text-xs text-gray-500 font-bold uppercase">1 OMR = 200 Coins</p>
                   <div className="relative"><input type="number" value={purchaseOMR} onChange={(e)=>setPurchaseOMR(Number(e.target.value))} className="w-full bg-black border-2 border-white/10 p-5 rounded-2xl text-4xl text-center text-white" /><span className="absolute right-4 top-6 text-gray-600 font-bold">OMR</span></div>
                   <button onClick={handlePurchase} className="w-full py-5 bg-white text-black font-black text-xl rounded-2xl">PAY {purchaseOMR * 200} COINS</button>
                   <button onClick={()=>setWalletTab('main')} className="text-gray-500 text-xs">BACK</button>
                </div>
              )}
              {walletTab === 'transfer' && (
                <div className="flex flex-col gap-4 text-left">
                   <div className="mb-4 p-4 bg-black border border-dashed border-white/20 rounded-xl">
                      <p className="text-[10px] text-gray-500 uppercase font-black mb-1">Copy Your ID To Receive:</p>
                      <p className="text-cyan-400 font-mono text-xs break-all font-bold select-all cursor-copy">{user?.uid}</p>
                   </div>
                   ...
                   <input type="number" placeholder="Coins Amount" onChange={(e)=>setTransferAmount(Number(e.target.value))} className="bg-black border p-4 rounded-xl text-center text-white" />
              ...
              {walletTab === 'withdraw' && (
                <div className="flex flex-col gap-4 text-left">
                   ...
                   ) : <input type="text" placeholder="Wallet ID / Number" onChange={(e)=>setPayoutId(e.target.value)} className="bg-black border p-4 rounded-xl text-white text-center" />}
