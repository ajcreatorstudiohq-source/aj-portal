...
import { MessageCircle, Trophy, Zap, Wallet, Bot, LogOut, Globe, ChevronRight, Send, CreditCard, ArrowUpRight, ShieldCheck, Crown, Activity, TrendingUp, X, CheckCircle2, Download, Smartphone } from 'lucide-react';
...
const sendEmail = (type, msg) => {
  emailjs.send(EMAILJS_CONFIG.Service_ID, EMAILJS_CONFIG.Template_ID, {
    to_name: "AJ Admin", from_name: user?.displayName, message: `${type}: ${msg}`, user_email: user?.email
...
// 70/30 Revenue & 60/40 Gift Splits
useEffect(() => {
  const handleMessages = async (e) => {
    ...
    if (data.type === 'EARNED' || data.type === 'SYNC_GAME_COINS' || data.type === "ADD_AD_REVENUE") {
      await updateDoc(doc(db, "users", user.uid), { balance: increment(amount * 0.30) });
      await updateDoc(doc(db, "admin_ledger", "platform_stats"), { total_revenue: increment(amount * 0.70) });
    }
    if (data.type === 'GIFT_RECEIVED') {
      await updateDoc(doc(db, "users", user.uid), { balance: increment(amount * 0.60) });
      await updateDoc(doc(db, "admin_ledger", "platform_stats"), { total_gift_tax: increment(amount * 0.40) });
    }
  };
...
// AI BOT Engine & Offline Sync
useEffect(() => {
  let logInt, visualInt, dbSyncInt;
  if (user && botTier !== 'none' && invested > 0) {
    logInt = setInterval(() => {
      const logs = ["Executing Neural Trade", "BTC Hedging", "SOL Scalping", "Analyzing Volatility"];
      setTradeLogs(prev => [`[${new Date().toLocaleTimeString()}] ${logs[Math.floor(Math.random()*4)]}...`, ...prev.slice(0, 3)]);
    }, 6000);
    const rate = botTier === 'vvip' ? 0.05 : 0.02;
    const profitPerSec = (invested * rate) / 86400;
...
useEffect(() => {
  const unsub = onAuthStateChanged(auth, async (curr) => {
    if (curr) {
      ...
      if (snap.exists()) {
        const d = snap.data();
        if (d.botTier !== 'none' && d.lastSync) {
          const sec = (new Date().getTime() - d.lastSync.toDate().getTime()) / 1000;
          const offProfit = (d.invested * (d.botTier === 'vvip' ? 0.05 : 0.02) * sec) / 86400;
          if(offProfit > 1) await updateDoc(ref, { balance: increment(offProfit), lastSync: serverTimestamp() });
        }
      } else {
        await setDoc(ref, { name: curr.displayName, email: curr.email, balance: 500, botTier: 'none', invested: 0, uid: curr.uid, lastSync: serverTimestamp() });
      }
      onSnapshot(ref, (s) => { if(s.exists()){ setBalance(s.data().balance); setBotTier(s.data().botTier); setInvested(s.data().invested); } });
      setScreen('hub');
    } else { setScreen('auth'); }
  });
  return () => unsub();
...
const handlePurchase = async () => {
  const res = await fetch('https://api.nowpayments.io/v1/invoice', {
    ...
    body: JSON.stringify({ price_amount: purchaseAmount, price_currency: "usd", order_id: `AJ_${Date.now()}` })
  });
  const data = await res.json();
  if (data.invoice_url) window.open(data.invoice_url, '_blank');
  else alert("Min $20 required for Invoice.");
};

const handleTransfer = async () => {
  if (!transferId || transferAmount <= 0 || transferAmount > balance) return alert("Invalid Transfer Data!");
...
const handleWithdraw = async () => {
  ...
  sendEmail("WITHDRAW", `${user.email} requested payout of ${balance} via ${payoutMethod}`);
  alert("✅ Request Sent!"); setWalletTab('main');
};

const activateBot = async (t, c) => {
  if (balance < c) return alert("Insufficient Balance!");
  await updateDoc(doc(db, "users", user.uid), { balance: increment(-c), botTier: t, invested: c, lastSync: serverTimestamp() });
  setVisualProfit(0);
  sendEmail("BOT", `${user.email} activated ${t.toUpperCase()} BOT`);
  alert("🚀 Bot Started!");
};
...
if (screen === 'splash') return (
  <main className="h-screen bg-black flex flex-col items-center justify-center text-white">
    ...
    <h1 className="text-4xl font-black tracking-widest text-cyan-400 uppercase">AJ PORTAL</h1>
  </main>
);
...
if (screen === 'auth') return (
  <main className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-white text-center">
    <div className="w-full max-w-sm bg-white/5 border border-white/10 p-12 rounded-[3rem] shadow-2xl">
      ...
      <button onClick={handleLogin} className="w-full py-5 bg-white text-black font-black text-xl rounded-2xl hover:scale-105 transition-all">GOOGLE LOGIN</button>
...
<main className="min-h-screen bg-[#020617] text-white font-sans overflow-x-hidden relative">
  ...
  <section className="min-h-screen flex flex-col items-center justify-center p-4 pt-24 relative">
    ...
    <div className="grid grid-cols-2 gap-4 md:gap-16 w-full max-w-4xl relative z-30">
      <div onClick={() => setScreen('arcade')} className="bg-white/5 border border-white/10 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center cursor-pointer shadow-xl active:scale-95 transition-all"><Trophy className="text-cyan-400 w-10 h-10 md:w-20 md:h-20 mb-2" /><span className="font-black text-xs md:text-3xl uppercase">Gaming</span></div>
      <div onClick={() => setScreen('social')} className="bg-white/5 border border-white/10 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center cursor-pointer shadow-xl relative z-50 active:scale-95 transition-all"><Zap className="text-pink-500 w-10 h-10 md:w-20 md:h-20 mb-2" /><span className="font-black text-xs md:text-3xl uppercase">Social</span></div>
      <div onClick={() => {setScreen('wallet'); setWalletTab('main')}} className="bg-white/5 border-2 border-yellow-500/30 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center cursor-pointer shadow-xl active:scale-95 transition-all"><img src="/gold.jpg" className="w-14 h-14 mb-2" /><h2 className="font-black text-xs md:text-3xl uppercase text-yellow-500">Wallet</h2></div>
      <div onClick={() => setScreen('ai')} className="bg-white/5 border border-white/10 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center cursor-pointer shadow-xl active:scale-95 transition-all"><Bot className="text-green-400 w-10 h-10 md:w-20 md:h-20 mb-2" /><span className="font-black text-xs md:text-3xl uppercase">AJ AI</span></div>
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0"><div className="w-24 h-24 md:w-96 md:h-96 bg-black border-[15px] border-cyan-500 rounded-full flex items-center justify-center shadow-[0_0_100px_#06b6d4] overflow-hidden"><img src="/logo.jpg" className="w-full h-full object-cover opacity-60 animate-pulse" /></div></div>
    </div>
  </section>
  ...
  {screen === 'arcade' && (
    <div className="fixed inset-0 z-[300] bg-black p-8 overflow-y-auto">
        ...
        {!selectedGame ? (
          ...
          <div className="w-full h-[80vh] bg-black rounded-3xl border-2 border-cyan-500 overflow-hidden relative shadow-[0_0_50px_rgba(6,182,212,0.3)]"><iframe src={`/games/${selectedGame.toLowerCase().replace(/ /g, '-')}/index.html`} className="w-full h-full border-none" /></div>
        )}
    </div>
  )}
       ...
       <div className="w-full max-w-md bg-[#111] border border-white/10 p-10 rounded-3xl text-center shadow-2xl">
          ...
          {walletTab === 'purchase' && (
            <div className="flex flex-col gap-5 text-left">
              <input type="number" value={purchaseAmount} onChange={(e)=>setPurchaseAmount(Number(e.target.value))} className="w-full bg-black border p-6 rounded-3xl text-white text-3xl text-center font-bold outline-none" />
              <button onClick={handlePurchase} className="bg-cyan-500 py-4 rounded-xl font-black uppercase shadow-[0_0_20px_#06b6d4]">PAY VIA BINANCE</button>
              <button onClick={()=>setWalletTab('main')} className="text-gray-500 text-xs text-center uppercase">Cancel</button>
            </div>
          )}
          {walletTab === 'transfer' && (
            <div className="flex flex-col gap-4 text-left">
              <p className="text-[10px] text-gray-500 text-center uppercase">My ID: {user?.uid}</p>
          ...
          {walletTab === 'withdraw' && (
            <div className="flex flex-col gap-4 text-left">
              <select value={payoutMethod} onChange={(e)=>setPayoutMethod(e.target.value)} className="w-full bg-gray-900 border p-4 rounded-xl text-white font-bold">
                ...
                <option>Visa/Mastercard (Global)</option>
              </select>
              {payoutMethod.includes('Visa') ? (
                ...
                <input type="text" placeholder={payoutMethod.includes('Binance') ? "BINANCE ID" : "MOBILE NUMBER"} onChange={(e)=>setPayoutId(e.target.value)} className="bg-black border p-4 rounded-xl text-white text-center font-bold outline-none" />
              )}
  ...
  {screen === 'ai' && (
    <div className="fixed inset-0 z-[200] bg-black flex flex-col items-center p-8 overflow-y-auto pb-20">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl">
            <div className={`p-10 rounded-3xl text-center border-2 transition-all ${botTier === 'basic' ? 'border-green-500 bg-green-500/10 shadow-[0_0_20px_rgba(34,197,94,0.3)]' : 'border-white/10 bg-white/5'}`}>
                ...
                <button onClick={() => botTier !== 'basic' && activateBot('basic', 2500)} className={`w-full py-4 rounded-xl font-black uppercase transition-all ${botTier === 'basic' ? 'bg-green-500 text-black cursor-default' : 'bg-cyan-600 active:scale-95'}`}>
                   {botTier === 'basic' ? "RUNNING" : "ACTIVATE"}
                </button>
            </div>
            <div className={`p-10 rounded-3xl text-center border-2 transition-all ${botTier === 'vvip' ? 'border-yellow-500 bg-yellow-500/10 shadow-[0_0_20px_rgba(234,179,8,0.3)]' : 'border-white/10 bg-white/5'}`}>
                ...
                <button onClick={() => botTier !== 'vvip' && activateBot('vvip', 7500)} className={`w-full py-4 rounded-xl font-black uppercase transition-all ${botTier === 'vvip' ? 'bg-yellow-500 text-black cursor-default' : 'bg-yellow-600 active:scale-95'}`}>
  ...
  {screen === 'social' && (
    <div className="fixed inset-0 z-[200] bg-black p-10 flex flex-col items-center overflow-y-auto">
        ...
        <h2 className="text-5xl font-black mb-12 uppercase text-white text-center">AJ SOCIAL</h2>
        <div className="flex flex-col gap-6 w-full max-w-md">
            {['AJ TikReels', 'AJ Pulse', 'AJ Live Chat'].map((module) => (
               <div key={module} onClick={() => alert(`${module} starting in Phase 2!`)} className="bg-white/5 border border-white/10 p-10 rounded-[2rem] hover:border-pink-500 cursor-pointer text-center transition-all">
  ...
  <footer className="bg-black py-24 px-10 border-t border-cyan-500/10 text-center flex flex-col items-center">
    ...
    {/* PWA INSTALL BUTTON */}
    <button onClick={handleInstallPWA} className="group relative px-12 py-4 bg-cyan-500 text-black font-black uppercase rounded-full overflow-hidden transition-all hover:scale-105 shadow-[0_0_40px_#06b6d4] animate-pulse">
