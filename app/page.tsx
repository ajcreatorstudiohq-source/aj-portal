...
// --- ALI BHAI'S 500:1 WITHDRAW MATH (1000 per $2) ---
...
// --- DYNAMIC PURCHASE HANDLER ($1 = 100 Coins) ---
...
<div onClick={() => {setScreen('wallet'); setWalletTab('main')}} className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-full border border-white/10 cursor-pointer transition-all hover:bg-white/10 shadow-[0_0_15px_rgba(0,255,255,0.1)]">
        ...
        {socialScreen === 'hub' ? (
          <div className="max-w-md mx-auto grid grid-cols-1 gap-6 pb-24 px-2 text-center">
             <div className="flex items-center gap-3 bg-white/5 p-4 rounded-3xl border border-pink-500/20 mb-4">
                  ...
                  <div><p className="font-black text-white text-sm uppercase">@{username || "AJ_Member"}</p><p className="text-[9px] text-gray-500 uppercase tracking-widest">{hasSocialProfile ? "Verified Member" : "Profile Not Set"}</p></div>
             </div>
          ...
          <div className="max-w-md mx-auto bg-white/5 border border-white/10 p-10 rounded-[3rem] text-center mt-4 shadow-2xl">
              ...
              {!user ? (
                  <div className="space-y-4">
                      ...
                      <button onClick={handleGoogleLogin} className="w-full py-4 bg-pink-600 font-black rounded-xl uppercase shadow-md">Google Login</button>
                  </div>
              ) : (
