...
// --- NO-RELOAD SPLASH LOGIC ---
useEffect(() => {
    if (screen === 'splash') {
        const interval = setInterval(() => { setLoading(prev => (prev >= 100 ? 100 : prev + 4)); }, 50);
        return () => clearInterval(interval);
    }
}, [screen]);

useEffect(() => {
    if (loading >= 100 && screen === 'splash') {
...
const handleSignOut = async () => { await signOut(auth); setScreen('auth'); setSocialScreen('hub'); };
...
{screen === 'social' && (
    <div className="fixed inset-0 z-[400] bg-slate-950 flex flex-col h-screen overflow-hidden">
        <header className="sticky top-0 w-full p-4 bg-black/90 backdrop-blur-md border-b border-white/10 flex justify-between items-center z-[500] rounded-b-3xl shrink-0 shadow-2xl">
            ...
            <h2 className="text-4xl font-black italic text-pink-500 uppercase text-center flex-1 tracking-tighter drop-shadow-[0_0_15px_#ec4899] animate-pulse font-orbitron text-shadow-[0_0_10px_#ec4899]">Dashboard</h2>
        ...
        {socialScreen === 'hub' ? (
          ...
          <div className="max-w-md mx-auto p-10 flex flex-col gap-6">
             ...
             <button onClick={() => setSocialScreen('setup')} className="bg-white/5 border border-white/10 p-6 rounded-3xl flex items-center gap-4 hover:bg-white/10 transition-all shadow-xl text-white font-black uppercase">Edit Profile</button>
             <button onClick={handleSignOut} className="bg-red-500/10 border border-red-500/20 p-6 rounded-3xl flex items-center gap-4 hover:bg-red-500/20 transition-all shadow-xl text-red-500 font-black uppercase tracking-widest">Sign Out / Switch</button>
             <button onClick={() => setSocialScreen('hub')} className="text-gray-500 uppercase text-[10px] font-black mt-10">Cancel</button>
          </div>
            ...
            <div className="h-full w-full max-w-md mx-auto snap-y snap-mandatory overflow-y-auto bg-black">
                {pixaVideos.map((vid:any, i) => (
                    <React.Fragment key={i}>
                        ...
                        {(i + 1) % 5 === 0 && <div onClick={()=>(window as any).AJ_SDK?.showAd()} className="h-[85vh] w-full snap-start flex items-center justify-center bg-gray-900 text-cyan-400 font-black flex-col gap-4 cursor-pointer shadow-2xl border-y-2 border-cyan-500/20"><VideoIcon size={70}/> <p className="uppercase tracking-[0.3em]">AJ VVIP VIDEO AD</p></div>}
                    </React.Fragment>
                ))}
            </div>
        ) : socialScreen === 'pulse' ? (
            ...
            <div className="max-w-md mx-auto bg-[#111b21]/80 backdrop-blur-2xl h-screen border-x border-white/10 shadow-2xl overflow-y-auto">
                ...
                <div className="mt-2 space-y-1">{['AJ Support', 'VIP Elite Club', 'Family WeChat'].map((contact, i) => (<div key={i} onClick={()=>{setActiveContact(contact); setSocialScreen('chat'); (window as any).AJ_SDK?.showAd();}} className="flex items-center gap-4 p-5 hover:bg-white/5 cursor-pointer border-b border-white/5 transition-all mx-2 rounded-[2rem]"><div className="w-14 h-14 rounded-full bg-cyan-600/30 flex items-center justify-center font-black border border-cyan-500/50 text-cyan-400 shadow-xl">AJ</div><div className="flex-1 text-left"><p className="font-black text-[#e9edef] uppercase text-xs">{contact}</p><p className="text-[10px] text-[#8696a0]">Secure chat active.</p></div></div>))}</div>
            </div>
        ) : socialScreen === 'chat' ? (
            <div className="max-w-md mx-auto h-[88vh] flex flex-col bg-[#0b141a] overflow-hidden m-2 rounded-[2.5rem] shadow-2xl border border-cyan-500/20">
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-[url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')] bg-repeat bg-contain opacity-90">{chatMessages.map((m:any) => (<div key={m.id} className={`flex ${m.uid === user.uid ? 'justify-end' : 'justify-start'}`}><div className={`p-3 max-w-[85%] rounded-2xl shadow-xl relative border ${m.uid === user.uid ? 'bg-cyan-700/80 border-cyan-400 text-[#e9edef] rounded-tr-none' : 'bg-[#202c33]/90 border-white/5 text-[#e9edef] rounded-tl-none'} backdrop-blur-md`}><p className="font-black text-[9px] text-yellow-500 mb-1 opacity-70 uppercase">@{m.username}</p><p className="text-[11px] leading-relaxed mb-1 pr-6 font-medium text-white">{m.text}</p><p className="text-[7px] text-white/30 absolute bottom-1.5 right-2 uppercase font-black tracking-widest">Now</p></div></div>))}</div>
...
{/* WALLET MODAL */}
{screen === 'wallet' && (
    <div className="fixed inset-0 z-[300] bg-black/98 flex flex-col items-center p-8 overflow-y-auto h-screen">
       ...
       <div className="w-full max-w-md bg-[#111] border border-white/10 p-10 rounded-3xl text-center shadow-2xl">
          ...
          {walletTab === 'transfer' && (
            <div className="flex flex-col gap-5 text-left">
                ...
                <input type="number" placeholder="Amount Coins" value={transferAmount === 0 ? '' : transferAmount} onChange={(e)=>setTransferAmount(e.target.value === '' ? 0 : Number(e.target.value))} className="bg-black/40 border border-white/10 p-4 rounded-2xl font-bold text-white outline-none focus:border-cyan-500 shadow-inner" />
          ...
          {walletTab === 'withdraw' && (
            <div className="flex flex-col gap-5 text-left">
                ...
                <input type="text" placeholder="Account Details (Phone/ID)" value={payoutId} onChange={(e)=>setPayoutId(e.target.value)} className="bg-black/40 border border-white/10 p-4 rounded-2xl font-bold text-white outline-none focus:border-pink-500 shadow-inner" />
    ...
    <div className="fixed inset-0 z-[600] bg-black flex flex-col items-center p-8 overflow-y-auto">
       ...
       {botTier !== 'none' && (
         <div className="w-full max-w-2xl bg-white/5 border-2 border-green-500/40 p-8 rounded-[3.5rem] text-center mb-16 shadow-[0_0_50px_rgba(34,197,94,0.15)] backdrop-blur-md">
            ...
            <div className="w-full bg-black/50 border border-green-500/30 p-8 rounded-3xl font-mono text-left shadow-inner"><div className="flex justify-between items-center mb-6"><span className="text-green-400 font-black text-xs uppercase tracking-widest">Neural Profit:</span><span className="text-white font-black text-2xl">+{visualProfit.toFixed(4)} 🪙</span></div><div className="h-24 overflow-hidden text-green-500/60 mt-2 text-[10px] leading-relaxed italic">Scanning market data... Trading engine active.</div></div>
         </div>
       )}
