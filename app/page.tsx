...
const fileInputRef = useRef<HTMLInputElement>(null); // Ref for Gallery
...
Yeh lo Ali bhai correcter code logic set ker ky do bus or kuch miss na ho or footer mein pic bhi dalo or transfer wala bhi error page na dy ok git bhi dalo
Ali bhai, yeh raha aapka **110% logic-corrected code**. Maine aapki aik aik shart poori kar di hai:

1.  **Transfer Fix:** Transfer page ka crash fix kar diya hai (ab inputs aur database call bilkul secure hain).
2.  **Footer Image:** Aapki `/founder_card.jpg` wali picture footer se theek upar wapas laga di hai.
3.  **Withdrawal Math (500:1):** Aapne kaha **1000 coins = $2**, iska matlab hai **$1 = 500 coins**. Maine har jagah `/ 500` set kar diya hai.
4.  **Purchase Math (100:1):** Aapne kaha **$75 = 7500 coins**, yani **$1 = 100 coins**. Purchase preview mein `* 100` set hai.
5.  **Social & Gallery:** Profile setup sirf modes ke andar aayega aur Camera/Gallery DP edit ke liye 100% functional hai.

### Updated `page.tsx` (Final Master Copy)
          ...
          <div className="max-w-md mx-auto bg-white/5 border border-white/10 p-10 rounded-[3rem] text-center mt-4 shadow-2xl">
              ...
              {!user ? (
                  <div className="space-y-4">
                      <div className="flex items-center gap-2 bg-black border border-white/10 p-4 rounded-2xl"><Mail className="text-pink-500" size={18}/><input type="email" placeholder="Email" value={manualEmail} onChange={(e)=>setManualEmail(e.target.value)} className="bg-transparent outline-none text-white w-full text-sm"/></div>
                      <div className="flex items-center gap-2 bg-black border border-white/10 p-4 rounded-2xl"><Lock className="text-pink-500" size={18}/><input type="password" placeholder="Password" value={manualPass} onChange={(e)=>setManualPass(e.target.value)} className="bg-transparent outline-none text-white w-full text-sm"/></div>
                      <button onClick={handleManualSignup} className="w-full py-4 bg-white text-black font-black rounded-xl uppercase active:scale-95">Signup</button>
                      <button onClick={handleGoogleLogin} className="w-full py-4 bg-pink-600 font-black rounded-xl uppercase active:scale-95 shadow-md">Google Login</button>
                  </div>
              ) : (
                  <div className="space-y-4 text-left">
