...
import { MessageCircle, Trophy, Zap, Wallet, Bot, Send, Activity, CheckCircle2, Download, Copy, Video, Newspaper, Users, Heart, MessageSquare, Camera, Settings, Edit3, X, Mail, Lock, User, Globe, DollarSign } from 'lucide-react';
...
const fileInputRef = useRef<HTMLInputElement>(null); // Gallery Ref
...
// Input States
const [purchaseAmount, setPurchaseAmount] = useState(20); // Minimum $20
...
const safePortalCoins = rawReward / 100; // 100:1 Game conversion
...
if (userSnap.exists()) {
  ...
  await setDoc(userRef, { name: currentUser.displayName || "User", email: currentUser.email, balance: 500, botTier: 'none', invested: 0, uid: currentUser.uid, lastSync: serverTimestamp(), hasSocialProfile: false, photo: currentUser.photoURL });
}
...
const handlePurchase = async () => {
  ...
  if (purchaseMethod === 'Binance (TRC20)') {
      ...
      await addDoc(collection(db, "manual_deposits"), { uid: user!.uid, email: user!.email, amount_usd: purchaseAmount, coins_awarded: (purchaseAmount * 100), method: "Airtm", txId: purchaseTxId, status: "pending", date: serverTimestamp() });
       ...
       <div className="w-full max-w-md bg-[#111] border border-white/10 p-10 rounded-3xl text-center shadow-2xl">
          ...
          {walletTab === 'purchase' && (
            <div className="flex flex-col gap-5 text-left">
              ...
              <button onClick={handlePurchase} className="bg-cyan-500 py-5 rounded-xl font-black uppercase shadow-lg shadow-cyan-500/20 active:scale-95 transition-all">Confirm Purchase</button>
              <button onClick={()=>setWalletTab('main')} className="text-gray-500 text-xs text-center uppercase hover:text-white">Cancel</button>
            </div>
          )}
          {/* Transfer & Withdraw sections remain same... */}
          {walletTab === 'transfer' && (
        ...
        {socialScreen === 'hub' ? (
          <div className="max-w-md mx-auto grid grid-cols-1 gap-6 pb-24 px-2">
             <div className="flex items-center gap-3 bg-white/5 p-4 rounded-3xl border border-pink-500/20 mb-4">
                  ...
                  <div><p className="font-black text-white text-sm uppercase">@{username || "AJ_Member"}</p><p className="text-[9px] text-gray-500 uppercase tracking-widest">{hasSocialProfile ? "Verified Member" : "Setup Identity"}</p></div>
             </div>
...
{/* Arcade Modal and Footer remain same... */}
