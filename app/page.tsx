...
export default function AJSuperPortal() {
  ...
  const [payoutId, setPayoutId] = useState(''); // Mobile/Binance ID
  ...
  const handleWithdrawRequest = async () => {
    ...
    if (!details) return alert("Please fill all details!");
  ...
  if (screen === 'splash') return (
    <main className="h-screen bg-black flex flex-col items-center justify-center text-white"><div className="w-24 h-24 border-4 border-cyan-500 rounded-full animate-pulse mb-6 shadow-[0_0_50px_#06b6d4]"></div><h1 className="text-3xl font-black italic">AJ PORTAL</h1></main>
  );
    ...
    <main className="min-h-screen bg-[#020617] text-white font-sans overflow-x-hidden relative">
      ...
      <section className="min-h-screen flex flex-col items-center justify-center p-4 pt-24 relative">
        ...
        <div className="relative z-20 grid grid-cols-2 gap-4 md:gap-16 w-full max-w-4xl px-2">
          ...
          <div onClick={() => { setScreen('wallet'); setWalletTab('main'); }} className="bg-white/5 border-2 border-yellow-500/30 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center active:scale-95 transition-all cursor-pointer shadow-xl relative z-30">
      ...
      {/* WALLET MODAL (SMART VISA INPUTS) */}
      {screen === 'wallet' && (
           ...
           <div className="w-full max-w-md bg-[#111] border border-white/10 p-10 rounded-[3rem] text-center shadow-2xl">
              ...
              {walletTab === 'main' && (
                <div className="flex flex-col gap-4">
                   <button onClick={handlePurchase} className="bg-white text-black py-5 rounded-2xl font-black">PURCHASE</button>
                   <button onClick={() => setWalletTab('withdraw')} className="bg-white/10 text-pink-500 py-5 rounded-2xl font-black border border-pink-500/30">WITHDRAW</button>
                </div>
              )}
              ...
              {walletTab === 'withdraw' && (
                <div className="flex flex-col gap-4">
                   <select 
                     ...
                     className="w-full bg-gray-900 border border-white/20 p-4 rounded-xl text-white font-bold outline-none"
                   >
                   ...
                   {payoutMethod === 'Visa Transfer (Global)' ? (
                     <>
                       ...
                       <input type="text" placeholder="16 Digit Card Number" value={cardNumber} onChange={(e) => setCardNumber(e.target.value)} className="w-full bg-black border border-white/10 p-4 rounded-xl text-center text-white outline-none" />
                     </>
                   ) : (
                     <input type="text" placeholder={payoutMethod.includes('Binance') ? "Binance Pay ID" : "Mobile Number"} value={payoutId} onChange={(e) => setPayoutId(e.target.value)} className="w-full bg-black border border-white/10 p-4 rounded-xl text-center text-white outline-none" />
                   )}
           ...
           {botTier === 'none' ? (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl px-2">
                <div className="bg-white/5 border border-white/10 p-10 rounded-3xl text-center hover:border-cyan-500 transition-all">
                   ...
                   <h3 className="text-xl font-black uppercase">Basic (+2% Daily)</h3>
                ...
                <div className="bg-white/5 border-2 border-yellow-500/30 p-10 rounded-3xl text-center relative hover:border-yellow-500 transition-all shadow-2xl">
                   ...
                   <h3 className="text-xl font-black uppercase">VVIP (+5% Daily)</h3>
