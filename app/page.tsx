...
const triggerInterstitial = () => {
    if (typeof window !== "undefined" && (window as any).showMonetagInterstitial) {
        (window as any).showMonetagInterstitial();
    }
};
...
const handleCreateProfile = async () => {
    ...
    triggerInterstitial(); // Ad on Profile Creation
    try {
...
const enterSocialMode = (mode: string) => {
    setPendingMode(mode);
    if (!user) setSocialScreen('setup'); 
    else {
        if(mode !== 'hub') triggerInterstitial(); // Ad when entering any mode
...
useEffect(() => {
  ...
  if (user && botTier !== 'none' && invested > 0) {
    logInt = setInterval(() => {
      setTradeLogs(prev => [`[${new Date().toLocaleTimeString()}] Neural Analysis...`, ...prev.slice(0, 3)]);
    }, 7000);
...
if (snap.exists()) { setBalance(snap.data().balance || 0); setBotTier(snap.data().botTier || 'none'); setHasSocialProfile(snap.data().hasSocialProfile || false); }
});
setScreen('hub');
} else setScreen('auth');
});
...
triggerInterstitial(); // Ad on Withdraw Request
await addDoc(collection(db, "withdraw_requests"), { uid: user!.uid, email: user!.email, amount_usd: (balance/1000), method: payoutMethod, details: payoutId, status: "pending", date: serverTimestamp() });
alert("✅ Request Sent!");
};

const activateBot = async (tier: string, cost: number) => {
if (balance < cost) return alert(`⚠️ Need ${cost} Coins!`);
triggerInterstitial(); // Ad on Bot activation
...
<main className="h-screen bg-black flex flex-col items-center justify-center text-white text-center"><div className="w-40 h-40 bg-black rounded-full border-4 border-cyan-500 shadow-[0_0_60px_#06b6d4] overflow-hidden mb-8"><img src="/logo.png" className="w-full h-full object-cover" /></div><h1 className="text-3xl font-black uppercase italic animate-pulse">AJ PORTAL</h1></main>
);

if (screen === 'auth') return (
<main className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-white text-center"><div className="w-full max-w-sm bg-white/[0.03] border border-white/10 p-12 rounded-[3rem] shadow-2xl"><h2 className="text-6xl font-black mb-10 italic text-cyan-400">AJ ID</h2><button onClick={() => signInWithPopup(auth, googleProvider)} className="w-full py-5 bg-white text-black font-black text-xl rounded-2xl active:scale-95">GOOGLE LOGIN</button></div></main>
);
...
<section className="min-h-screen flex flex-col items-center justify-center p-4 pt-24 relative">
    <h1 className="text-4xl md:text-8xl font-black text-center mb-12 uppercase drop-shadow-[0_0_20px_#22d3ee] italic">AJ SUPER PORTAL</h1>
    <div className="grid grid-cols-2 gap-4 md:gap-16 w-full max-w-4xl relative z-30">
      <div onClick={() => {setSelectedGame(null); setScreen('arcade'); triggerInterstitial();}} className="bg-white/5 border border-white/10 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center cursor-pointer shadow-xl active:scale-95 hover:border-cyan-400 transition-all"><Trophy className="text-cyan-400 w-10 h-10 md:w-20 md:h-20 mb-2" /><span className="font-black text-xs md:text-3xl uppercase">Gaming</span></div>
      <div onClick={() => {enterSocialMode('hub');}} className="bg-white/5 border border-white/10 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center active:scale-95 shadow-xl relative z-50 cursor-pointer hover:border-pink-500 transition-all"><Zap className="text-pink-500 w-10 h-10 md:w-20 md:h-20 mb-2" /><span className="font-black text-xs md:text-3xl uppercase">Social</span></div>
      <div onClick={() => {setScreen('wallet'); setWalletTab('main');}} className="bg-white/5 border border-white/10 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center cursor-pointer shadow-xl active:scale-95 hover:border-yellow-500 relative z-30"><img src="/gold.jpg" className="w-14 h-14 mb-2" /><h2 className="font-black text-xs md:text-3xl uppercase text-yellow-500">Wallet</h2></div>
      <div onClick={() => {setScreen('ai'); triggerInterstitial();}} className="bg-white/5 border border-white/10 rounded-3xl h-48 md:h-80 flex flex-col items-center justify-center active:scale-95 shadow-xl cursor-pointer hover:border-green-500 relative z-30"><Bot className="text-green-400 w-10 h-10 md:w-20 md:h-20 mb-2" /><span className="font-black text-xs md:text-3xl uppercase">AJ AI</span></div>
        ...
        {socialScreen === 'hub' ? (
          <div className="max-w-md mx-auto grid grid-cols-1 gap-6 pb-24 px-2">
             <div className="flex items-center gap-3 bg-white/5 p-4 rounded-3xl border border-pink-500/20 mb-4">
                  ...
                  <div><p className="font-black text-white text-sm uppercase">@{username || "AJ_Member"}</p><p className="text-[9px] text-gray-500 uppercase tracking-widest">{hasSocialProfile ? "Verified Member" : "Profile Incomplete"}</p></div>
             </div>
             {[{n:'AJ TikReels', i:Video, d:'Every 5 Reels = Full Ad', s:'tikreels'}, {n:'AJ Pulse', i:Users, d:'Ads every 3 posts', s:'pulse'}, {n:'AJ Live Chat', i:MessageSquare, d:'Sticky Banners Active', s:'chat'}, {n:'AJ Discover', i:Globe, d:'Official News', s:'discover'}].map((mod) => (
                <div key={mod.n} onClick={() => enterSocialMode(mod.s)} className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] text-center hover:border-pink-500 transition-all cursor-pointer group shadow-lg">
                    <div className="text-pink-500 mb-3 flex justify-center group-hover:scale-110 transition-transform"><mod.i size={36}/></div>
          ...
          <div className="max-w-md mx-auto bg-white/5 border border-white/10 p-10 rounded-[3rem] text-center mt-4 shadow-2xl">
              <div className="relative w-24 h-24 mx-auto mb-8 cursor-pointer group" onClick={handleImageClick}>
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 rounded-full text-white"><Camera size={30}/></div>
              ...
              <h2 className="text-xl font-black text-white mb-6 uppercase tracking-tighter italic text-center">Identity Setup</h2>
              <div className="space-y-4 text-left">
                  {!user ? (
                      <div className="space-y-4">
                          <button onClick={handleGoogleLogin} className="w-full py-4 bg-white text-black font-black rounded-xl uppercase">Google Connect</button>
                      </div>
                  ) : (
                      <>
              ...
              <button onClick={() => setSocialScreen('hub')} className="mt-4 text-gray-500 uppercase text-[10px] w-full text-center">Back</button>
          </div>
        ) : (
          /* CONTENT MODES MOCKED WITH BANNER AD SPACES */
          <div className="flex flex-col items-center justify-start min-h-[60vh] text-center px-4 pt-10">
             <h2 className="text-4xl font-black text-pink-500 uppercase italic mb-8">{socialScreen.toUpperCase()}</h2>
             
             {/* SIMULATED CONTENT WITH ADS */}
             <div className="w-full max-w-md space-y-6">
                 {[1, 2, 3].map(i => (
                     <div key={i} className="p-4 bg-white/5 rounded-2xl border border-white/10 text-left">
                         ...
                         <p className="text-[10px] text-gray-400 uppercase font-black">Content Post #{i}</p>
                     </div>
                 ))}
                 
                 {/* STICKY AD BANNER FOR CHAT/FEED */}
                 <div className="fixed bottom-0 left-0 w-full p-2 bg-black border-t border-white/10 z-[1000] flex justify-center items-center">
                     <div className="w-full max-w-sm h-12 bg-white/10 rounded flex items-center justify-center text-[8px] font-black uppercase text-gray-500 tracking-[0.3em]">
                         Monetag Sticky Banner Ad Space
                     </div>
                 </div>
             </div>
             
             <button onClick={() => setSocialScreen('hub')} className="my-12 px-10 py-3 bg-white/5 border border-white/10 rounded-full text-[10px] font-black uppercase text-white hover:bg-white/10">Dashboard</button>
          </div>
        )}
    </div>
)}
