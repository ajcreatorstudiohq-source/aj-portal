...
const handleDeletePost = async (id: string) => { if (confirm("Delete this post permanently?")) { await deleteDoc(doc(db, "user_posts", id)); setActiveMenuId(null); } };
...
{screen === 'social' && (
    <div className="fixed inset-0 z-[400] bg-slate-950 flex flex-col h-screen overflow-hidden">
        ...
        {commentBoardPostId && (
            <div className="fixed inset-0 z-[1000] bg-black/60 backdrop-blur-md flex items-end">
                <div className="w-full h-[65vh] bg-[#111b21] rounded-t-[3rem] border-t-2 border-pink-500 p-6 flex flex-col shadow-[0_-20px_50px_rgba(236,72,153,0.2)]">
                    ...
                    <div className="mt-4 pt-4 border-t border-white/10 flex gap-3"><input type="text" value={newComment} onChange={(e)=>setNewComment(e.target.value)} placeholder="Write a comment..." className="flex-1 bg-black/40 border border-white/10 rounded-xl p-4 text-xs outline-none focus:ring-1 focus:ring-pink-500 text-white" /><button onClick={submitComment} className="bg-pink-600 p-4 rounded-xl shadow-lg active:scale-90 transition-all"><Send size={18} className="text-white"/></button></div>
                </div>
            </div>
        )}
    </div>
...
<footer className="bg-black py-24 px-10 border-t border-white/5 text-center flex flex-col items-center relative overflow-hidden">
    ...
    <div className="mt-16 pt-16 border-t border-white/10 w-full text-center">
